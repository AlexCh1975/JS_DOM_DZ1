...
if (schedules){
    schedules.forEach(schedule => {
        renderScheduleHtml(schedule.id, schedule.name, schedule.dateTime, schedule.maxUsers, schedule.currentUsers);
    });
}

function renderScheduleHtml(id, name, dateTime, maxUsers, currentUsers){
    classes.insertAdjacentHTML("beforeEnd", 
        `
...
function maxValueCheck(elId){
    ...
    const currentUsers = +element.querySelector('.item__currentusers').textContent;
